...
import { DetailHero, Empty } from '@/components/launcher/detail';
import { contentIcon, contentKindLabel } from '@/components/launcher/icons';
import { Stat } from '@/components/launcher/page';
...
function ProjectDetailPage() {
  ...
  return (
    <div className="flex min-h-full flex-col">
      ...
      <div className="grid flex-1 gap-6 p-5 lg:grid-cols-[1fr_260px]">
                ...
                <div key={v.id} className="flex items-center gap-3 px-3 py-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="font-mono text-[11px] text-muted-foreground">
                      {v.loader} · {v.game} ·{' '}
                      {agoLabel(
                        Math.floor(Date.now() / 1000 - v.when * 86_400),
                      )}
                    </div>
                  </div>
