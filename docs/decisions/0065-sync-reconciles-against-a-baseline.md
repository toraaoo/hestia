...
one. The baseline lives inside the store it describes;
`remove_instance` drops what the shared store held for a deleted instance, the
one thing nothing else would collect.
