...
TEST(Checksum, EmptyInput) {
    EXPECT_EQ(digest(HashAlgorithm::sha1, ""), "da39a3ee5e6b4b0d3255bfef95601890afd80709");
    EXPECT_EQ(digest(HashAlgorithm::sha256, ""), "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
}

TEST(Checksum, Abc) {
    EXPECT_EQ(digest(HashAlgorithm::sha1, "abc"), "a9993e364706816aba3e25717850c26c9cd0d89d");
    EXPECT_EQ(digest(HashAlgorithm::sha256, "abc"), "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
}

TEST(Checksum, MillionAs) {
    const std::string input(1000000, 'a');
    EXPECT_EQ(digest(HashAlgorithm::sha1, input), "34aa973cd4c4daa4f61eeb2bdbad27316534016f");
    EXPECT_EQ(digest(HashAlgorithm::sha256, input), "cdc76e5c9914fb9281a1c7e284d73e67f1809a48a497200e046d39ccc7112cd0");
}
