...
use crate::runtime::{instance_process_id, Channels, ContentJob, ServiceError};
...
pub(super) fn register(on: &mut Channels<'_>) {
    ...
    on.handle::<ProfileCreate, _, _>(|p, ctx| async move {
        if p.name.trim().is_empty() {
            return Err(ServiceError::bad_request("a profile name is required"));
        }
        ctx.runtime
            .engine()
            .profiles()
    ...
    on.handle::<ProfileEdit, _, _>(|p, ctx| async move {
        if p.add.is_empty() && p.remove.is_empty() {
            return Err(ServiceError::bad_request("nothing to add or remove"));
        }
        ctx.runtime
            .engine()
