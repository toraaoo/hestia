...
mod tests {
    ...
    fn temp(tag: &str) -> tempfile::TempDir {
        tempfile::Builder::new()
            .prefix(&format!("hestia-mrpack-{tag}-"))
            .tempdir()
            .expect("temp dir")
    }
    ...
    fn a_curseforge_pack_uses_the_tree_its_manifest_names() {
        ...
        let dest_dir = temp("cf-overrides");
        let dest = dest_dir.path();
