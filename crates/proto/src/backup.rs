//! Backup contracts shared by the server and instance domains: archive an
//! entry's `data/` directory on demand, list/restore/remove the stored
//! archives. Create and restore are jobs — the call answers with the job id
//! and progress/done/error arrive as events, like the other provisioning
//! jobs. Servers additionally back up on a schedule and before a version
//! update — the schedule is configured through the server's `config` channels
//! (`backup-interval` / `backup-retention`), not a channel of its own.
...
#[derive(Serialize, Deserialize, Default, Debug, Clone)]
#[serde(default)]
pub struct InstanceBackupCreateParams {
    /// Instance name or id.
    pub instance: String,
    /// Client-supplied job id; empty asks the daemon to allocate one.
    pub id: String,
...
#[derive(Serialize, Deserialize, Default, Debug, Clone)]
#[serde(default)]
pub struct InstanceBackupRestoreParams {
    /// Instance name or id.
    pub instance: String,
    /// Backup id (`instance.backup.list` names them).
    pub backup: String,
    /// Client-supplied job id; empty asks the daemon to allocate one.
    pub id: String,
}

/// Names one backup of one instance (remove).
#[derive(Serialize, Deserialize, Default, Debug, Clone)]
#[serde(default)]
pub struct InstanceBackupRef {
    pub instance: String,
    pub backup: String,
}

pub struct InstanceBackupCreate;
impl Contract for InstanceBackupCreate {
    const CHANNEL: &'static str = "instance.backup.create";
    type Params = InstanceBackupCreateParams;
    type Result = BackupJobResult;
}

pub struct InstanceBackupList;
impl Contract for InstanceBackupList {
    const CHANNEL: &'static str = "instance.backup.list";
    type Params = InstanceRef;
    type Result = BackupListResult;
}

pub struct InstanceBackupRestore;
impl Contract for InstanceBackupRestore {
    const CHANNEL: &'static str = "instance.backup.restore";
    type Params = InstanceBackupRestoreParams;
    type Result = BackupJobResult;
}

pub struct InstanceBackupRemove;
impl Contract for InstanceBackupRemove {
    const CHANNEL: &'static str = "instance.backup.remove";
    type Params = InstanceBackupRef;
    type Result = Empty;
}
