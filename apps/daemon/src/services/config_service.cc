...
namespace hestia::daemon {
    void register_config_service(Router &router) {
        router.on("config.get", [](const ipc::Request &req, HandlerContext &ctx) {
            ...
            if (const auto value = ctx.engine.config().get(key)) {
