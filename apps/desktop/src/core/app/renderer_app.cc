...
namespace desktop::app {
    ...
    void RendererApp::OnContextCreated(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame,
                                       CefRefPtr<CefV8Context> context) {
        message_router_->OnContextCreated(browser, frame, context);
    }

    void RendererApp::OnContextReleased(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame,
                                        CefRefPtr<CefV8Context> context) {
        if (message_router_) message_router_->OnContextReleased(browser, frame, context);
    }

    bool RendererApp::OnProcessMessageReceived(CefRefPtr<CefBrowser> browser, CefRefPtr<CefFrame> frame,
        ...
        if (message->GetName() == "hestia.emit") {
            ...
            std::string js = "window.dispatchEvent(new CustomEvent('" + channel + "', { detail: " + payload + " }));";
            frame->ExecuteJavaScript(js, "", 0);
            return true;
        }
        if (message_router_) return message_router_->OnProcessMessageReceived(browser, frame, source_process, message);
        return false;
    }
