...
namespace desktop::ipc {
    ...
    std::string Request::PayloadString() const {
        ...
        if (v && v->GetType() == VTYPE_STRING) return v->GetString().ToString();
        return payload_raw;
    }
    ...
    bool Registry::Handle(const std::string &channel, const Request &req, Response res) const {
    ...
    Actions::Actions(std::string prefix, Registry &reg) : prefix_(std::move(prefix)), registry_(reg) {}
    ...
    namespace {
        ...
        public:
            bool OnQuery(CefRefPtr<CefBrowser> /*browser*/, CefRefPtr<CefFrame> /*frame*/, int64_t /*query_id*/,
                         const CefString &request_str, bool /*persistent*/,
                ...
                req.payload_raw = payload_v ? CefWriteJSON(payload_v, JSON_WRITER_DEFAULT).ToString() : "null";
    ...
    void Emit(CefRefPtr<CefBrowser> browser, const std::string &channel, const std::string &payload_json) {
        if (!browser) return;
