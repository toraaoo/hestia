...
func (sc *Commands) newRestartCmd() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return sc.withClient(cmd, func(c *client.Client) error {
				name := args[0]
				spinner := ui.NewSpinner(os.Stdout, fmt.Sprintf("Restarting %s...", name))
				spinner.Start()
				err := c.RestartServer(cmd.Context(), name)
				spinner.Stop()
				if err != nil {
					return err
				}
				fmt.Printf("%s Restarted server %s\n", ui.StateRunning.Render("✓"), name)
				return nil
			})
		},
	}
