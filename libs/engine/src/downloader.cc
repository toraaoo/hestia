...
namespace hestia::engine {
    ...
    namespace {
        ...
        void validate(const Checksum &checksum) {
            const auto expected_len = hex_digest_length(checksum.algorithm);
            if (checksum.hex.size() != expected_len || !is_hex(checksum.hex)) {
